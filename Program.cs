...
////作法一：
builder.Services.AddDbContext<MVC_UserDBContext>(options => options.UseSqlServer("Server=.\\sqlexpress;Database=MVC_UserDB;Trusted_Connection=True;TrustServerCertificate=true;MultipleActiveResultSets=true"));
...
//**** 讀取 appsettings.json 設定檔裡面的資料（資料庫連結字串）****
////作法一：
builder.Services.AddDbContext<MVC_UserLoginContext>(options => options.UseSqlServer("Server=.\\sqlexpress;Database=MVC_UserLogin;Trusted_Connection=True;TrustServerCertificate=true;MultipleActiveResultSets=true"));
//// 這裡需要新增兩個命名空間，請使用「顯示可能的修正」讓系統自己加上。
